...
export async function buildApp() {
  const config = loadConfig({ serviceName: 'notification-service', defaultPort: 3006 });
  const app = Fastify({
  ...
  app.decorate('notificationConsumerService', new NotificationConsumerService(config.kafkaBrokers));
